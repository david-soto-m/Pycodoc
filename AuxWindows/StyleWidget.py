...
class StyleWidget (QW.QWidget):
	...
	def showWid(self):
		...
		fnt=QW.QPushButton("Font")
		fnt.clicked.connect(self.fontLauncher)
		L.addWidget(fnt)
		
		bcl=QW.QPushButton("Background Colour")
		bcl.clicked.connect(self.bColorLauncher)
		L.addWidget(bcl)
		
		cl=QW.QPushButton("Text Colour")
		cl.clicked.connect(self.colorLauncher)
		L.addWidget(cl)
		
		bscl=QW.QPushButton("Selection Background Colour")
		bscl.clicked.connect(self.bSelColorLauncher)
		L.addWidget(bscl)
		
		scl=QW.QPushButton("Selection Text Colour")
	...
	def createHandle(self):
		if self.name.text()=="":
			QW.QMessageBox.question(self,'Error','You need a name for the style',QW.QMessageBox.Ok,QW.QMessageBox.Ok)
		else:
			stri="QTextEdit{\n"
			for item in self.dictio:
				stri+="\t%s: %s;\n"%(item,self.dictio[item])
			stri+="}"
			namePath=str(Path(GXML.GConfigRoot.find("StyleLocs/Path").text).parent)+"/"+self.name.text()+".css"
			print(namePath)
			elem=fileElement(namePath,style=True)
			xmly=elem.createFileElement()
			GXML.styleLocsRoot.append(xmly)
	...
	def bottomBar(self):
		applyBtn = QW.QPushButton('Create', self)
