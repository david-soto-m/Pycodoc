...
from MainWindow.ToolBar.ToolBar import opener
...
class MenuBar(QMenuBar):
    def __init__(self, parent):
        ...
        self.editFiles = QAction(
            QIcon().fromTheme('kt-queue-manager'),
            'Configure Files',
            parent
        )
        ...
        self.behaviourSettings = QAction(
            QIcon().fromTheme('settings-configure'),
            'Configure Pycodoc',
            parent
        )
        self.behaviourSettings.triggered.connect(
            self.behaviourManager.showWid
        )
