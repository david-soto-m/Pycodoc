...
class centralWidget(QWidget):
    ...
    def defineTabBar(self):
        ...
        TabBar.setTabBarAutoHide(
            BehaviourRoot.find('TabBarAutoHide').text not in self.remain_txt
        )
        return TabBar
        ...
        if type(files) == bool or files is None:
            ...
            for idx in range(self.CwidLayout.count()):
                self.CwidLayout.itemAt(idx).widget().addTab(
                    ...
                    fileElement().title.text
                )
        elif type(files) == fileElement:
            if (
                 BehaviourRoot.find('Pandoc').text in self.yes_txt
                 and not files.isFormat('.html')):
                boool = True
                if (
                     BehaviourRoot.find('Hpandoc').text in self.shortcuts_txt
                     and not pandoc):
                    boool = False
                if BehaviourRoot.find('Hpandoc').text in self.pop_txt:
                    mboxans = QMessageBox.question(
                        self.parent,
                        'Confirmation',
                ...
                if boool:
                    ...
                    if not BehaviourRoot.find(
                       'Hpandoc').text in self.create_txt:
            ...
            self.CwidLayout.itemAt(0).widget().setCurrentIndex(
                len(self.TabList)-1
            )
    ...
    def tabDestroyer(self, index=None):
        if self.CwidLayout.itemAt(0).widget().count() > 1:
            ...
            if (Behave in self.welcome_txt):
                for idx in range(self.CwidLayout.count()):
                    self.CwidLayout.itemAt(idx).widget().removeTab(0)
                self.TabList.pop(0)
                self.tabAdder()
            elif (Behave in self.none_txt):
                for idx in range(self.CwidLayout.count()):
                    self.CwidLayout.itemAt(idx).widget().removeTab(0)
                self.TabList.pop(0)
            elif (Behave in self.persist_txt):
                pass  # Not a mistake!!
            else:
                qApp.quit()
    ...
    def pandocize(self):
        ...
        if (
             BehaviourRoot.find('Hpandoc').text in self.shortcuts_txt
             and BehaviourRoot.find('Pandoc').text in self.yes_txt
             and not files.isFormat('.html')):
    ...
    def stylize(self, styleFile):
        ...
        for idx in range(self.CwidLayout.count()):
            for tabidx in range(self.CwidLayout.itemAt(idx).widget().count()):
                self.CwidLayout.itemAt(idx).widget().widget(tabidx).stylize(
                    styleFile
                )
